...
import { findFriendshipsByUserId, acceptFriendRequest } from "@/lib/repositories/friend.repository"
...
export async function POST(_req: Request, { params }: { params: Promise<{ friendId: string }> }) {
  ...
  if (friendship.addresseeId !== session.user.id) {
    return NextResponse.json({ error: "Você não pode aceitar este pedido" }, { status: 403 })
  }
  ...
  await createNotification({
    ...
    message: `${currentUser?.name || currentUser?.email} aceitou seu pedido de amizade.`,
  })
